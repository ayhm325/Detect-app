...
const RATE_LIMIT_CHAT_MAX = parseInt(process.env.RATE_LIMIT_CHAT_MAX || "") || 60;
const RATE_LIMIT_REDIS_RETRIES = parseInt(process.env.RATE_LIMIT_REDIS_RETRIES || "") || 3;
...
export async function rateLimit(request) {
  if (process.env.NODE_ENV === "development" && process.env.RATE_LIMIT_DISABLE_DEV !== "false") {
  ...
  for (let attempt = 1; attempt <= RATE_LIMIT_REDIS_RETRIES; attempt++) {
    try {
      ...
      console.warn(`Redis attempt ${attempt} failed for key ${key}:`, err.message);
  ...
  try {
    ...
    if (recent.length >= max) {
      console.info(`Memory fallback rate limit exceeded for ${key}: ${recent.length} > ${max}`);
      return { limited: true };
    }
    recent.push(now);
