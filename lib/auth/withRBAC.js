...
export function withRBAC(handler, allowedRoles) {
  return async function (request, ...args) {
    ...
    try {
      ...
      if (code === "FORBIDDEN_ROLE" || code === "INACTIVE_USER" || code === "DELETED_USER") {
        return NextResponse.json({ error: "Forbidden" }, { status: 403 });
      }
      return NextResponse.json({ error: "Authentication error" }, { status: 401 });
    }
    // Patch: unwrap params if present and is a Promise (Next.js App Router dynamic route)
    if (args && args.length > 0 && args[0] && typeof args[0].params === 'object' && typeof args[0].params.then === 'function') {
