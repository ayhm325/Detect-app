...
export async function getBillingsForPatient(patientId) {
  // Billing model not present in Prisma schema — return empty list
  console.warn('getBillingsForPatient: Billing model not defined in schema.prisma');
  return [];
}
...
export async function getAppointmentsForPatient(patientId) {
  return await prisma.appointment.findMany({
    where: { patientId },
    include: { doctor: { include: { user: true } }, medicalRecord: true }
  });
}
