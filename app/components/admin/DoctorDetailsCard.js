...
function DoctorDetailsCard({ doctor, onApprove, onReject, onClose }) {
  ...
  const displayName = doctor.name || doctor.fullName || (doctor.user && (doctor.user.name || doctor.user.fullName)) || ui("placeholder");
  const displayEmail = doctor.email || (doctor.user && doctor.user.email) || ui("placeholder");
  return (
    <div className="card-glass p-8 border border-(--ui-border) max-w-md mx-auto mt-8">
      ...
      {statusKey === "pending" && (
        <div className="flex gap-3 mt-6">
          <button className="flex-1 px-6 py-2 rounded-full bg-(--ui-success) text-white border border-(--ui-success-border) font-bold" onClick={() => onApprove?.(doctor)}>{t("approve")}</button>
          <button className="flex-1 px-6 py-2 rounded-full bg-(--ui-danger) text-white border border-(--ui-danger-border) font-bold" onClick={() => onReject?.(doctor)}>{t("reject")}</button>
        </div>
      )}
      <button className="mt-6 px-6 py-2 rounded-full bg-(--ui-surface-2) hover:bg-(--ui-surface) text-foreground border border-(--ui-border) font-bold w-full transition-colors" onClick={onClose}>{t("close")}</button>
    </div>
  );
}
