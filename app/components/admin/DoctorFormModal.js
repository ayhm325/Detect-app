...
export default function DoctorFormModal({ open, onClose, onSave, doctor }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <form className="bg-white dark:bg-zinc-900 p-8 rounded-2xl shadow-2xl min-w-[400px] max-w-[500px] w-full flex flex-col gap-5 border border-gray-200 dark:border-zinc-800" onSubmit={e => {e.preventDefault(); onSave({ name, email, specialty });}}>
        <h3 className="font-bold text-2xl mb-2 bg-gradient-to-r from-yellow-600 to-red-600 bg-clip-text text-transparent">{doctor ? "تعديل طبيب" : "إضافة طبيب جديد"}</h3>
