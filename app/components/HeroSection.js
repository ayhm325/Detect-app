...
export default function HeroSection() {
  ...
  return (
    <section className="relative flex flex-col items-center justify-center min-h-[90vh] w-full py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      {/* Gradient Overlay */}
      <div className="absolute inset-0 -z-20 pointer-events-none bg-gradient-to-br from-yellow-50/90 via-red-50/80 to-white/90 dark:from-zinc-900/90 dark:via-zinc-800/80 dark:to-zinc-900/90" />
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none">
        {particles.map((particle) => (
          <div
            key={particle.id}
            className="absolute rounded-full bg-gradient-to-br from-yellow-400/20 to-red-400/20 blur-xl animate-float"
            style={{
      ...
      <div className="relative z-10 flex gap-4 sm:gap-6 lg:gap-8 mb-12 flex-wrap justify-center">
        {[
          ...
          >
            <div className={`absolute inset-0 bg-gradient-to-br from-${icon.color}-400 to-${icon.color === 'yellow' ? 'red' : 'yellow'}-600 rounded-3xl blur-2xl opacity-40 group-hover:opacity-60 transition-opacity`} />
      ...
      <h1 className="relative z-10 text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-center mb-6 animate-fadeIn">
        <span className="block bg-gradient-to-r from-yellow-500 via-red-500 to-red-700 bg-clip-text text-transparent drop-shadow-2xl animate-gradient">
          {t.heroTitle || "الكشف الذكي عن الأمراض"}
        </span>
        <span className="block text-2xl sm:text-3xl md:text-4xl mt-4 bg-gradient-to-r from-red-600 to-yellow-600 bg-clip-text text-transparent">
      ...
      >
        {t.heroDesc || "نظام متطور لتحليل الصور الطبية وتشخيص الأمراض بدقة عالية باستخدام تقنيات الذكاء الاصطناعي"}
      </p>
      ...
      <div className="relative z-10 flex flex-col sm:flex-row gap-4 sm:gap-6 items-center justify-center animate-fadeIn" style={{ animationDelay: "0.4s" }}>
        ...
        >
          <span className="relative z-10 flex items-center gap-3">
            ...
            {t.signup || "ابدأ الآن مجاناً"}
          </span>
          <div className="absolute inset-0 bg-gradient-to-r from-red-600 to-yellow-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        </Link>
      ...
      <div className="relative z-10 mt-16 grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 w-full max-w-4xl animate-fadeIn" style={{ animationDelay: "0.6s" }}>
        {[
          ...
          >
            <div className="text-2xl sm:text-3xl font-black bg-gradient-to-r from-yellow-600 to-red-600 bg-clip-text text-transparent text-center mb-2">
              {stat.number}
            </div>
