  ...
  return (
    <section className="card-glass rounded-xl border border-(--ui-border) shadow-sm">
      <header className="flex items-center justify-between border-b border-(--ui-border) p-4">
        <h2 className="text-lg font-semibold text-(--ui-foreground)">{t("components.chat.title")}</h2>
      </header>
      <div className="flex h-96 flex-col">
        <div className="flex-1 space-y-2 overflow-y-auto p-4">
          {messages.map((m, i) => (
            ...
            >
              <div className="text-xs opacity-75">
                {m.role === "patient" ? t("components.chat.role.patient") : t("components.chat.role.doctor")}
              </div>
              <div>{m.text}</div>
            </div>
          ))}
          {typing && (
            <div className="inline-flex items-center gap-2 rounded-md bg-(--ui-surface-2)/60 px-3 py-2 text-sm text-(--ui-foreground)">
              ...
              <span className="inline-block h-2 w-2 animate-bounce rounded-full bg-(--ui-muted-foreground)/70" style={{ animationDelay: "0.1s" }} />
              <span className="inline-block h-2 w-2 animate-bounce rounded-full bg-(--ui-muted-foreground)/70" style={{ animationDelay: "0.2s" }} />
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            setTimeout(() => {
              setTyping(false);
              setMessages((prev) => [...prev, { role: "doctor", text: t("components.chat.mockReply") }]);
            }, 1000);
          }}
        >
          <input
          ...
          />
          <button className="btn-gradient rounded-md px-3 py-2 text-sm text-white">{t("components.chat.send")}</button>
        </form>
      </div>
    </section>
  );
