...
export default function AddAppointmentModal({ open, appointment, onClose, onSave }) {
  ...
  const normalizeStatus = (value) => {
    ...
    if (value === 'scheduled' || value === 'confirmed' || value === 'cancelled') return value;
  ...
  return (
    <div className={styles.overlay}>
      <div className={styles.modal}>
        <h2>{appointment ? t('calendar.modal.editTitle') : t('calendar.modal.addTitle')}</h2>
        <form onSubmit={handleSubmit}>
          ...
          <label>
            ...
            <select name="status" value={form.status} onChange={handleChange}>
              <option value="scheduled">{t('calendar.statusLabels.scheduled')}</option>
              <option value="confirmed">{t('calendar.statusLabels.confirmed')}</option>
              <option value="cancelled">{t('calendar.statusLabels.cancelled')}</option>
            </select>
          </label>
          <div className={styles.actions}>
            <button type="submit">{t('calendar.actions.save')}</button>
            <button type="button" onClick={onClose}>{t('calendar.actions.cancel')}</button>
          </div>
        </form>
      </div>
    </div>
