  ...
  return (
    <div className={styles.card}>
      <div className={styles.info}>
        <div>{t('calendar.card.patient')}: {appointment.patient}</div>
        <div>{t('calendar.card.time')}: {appointment.time}</div>
        <div>{t('calendar.card.status')}: {statusLabel}</div>
      </div>
      <button className={styles.editBtn} onClick={onEdit}>{t('calendar.actions.edit')}</button>
    </div>
  );
}
