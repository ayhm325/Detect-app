...
export const POST = withRBAC(async (request, user) => {
  ...
  try {
    ...
    const rawPassword = body.password || defaultPassword || (process.env.NODE_ENV === 'development' ? 'changeme' : null);
    if (!rawPassword) {
      return new Response(JSON.stringify({ error: 'Missing password (set DEFAULT_PATIENT_PASSWORD in production)' }), { status: 400 });
    }
