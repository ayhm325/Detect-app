...
export const POST = withRBAC(async (request, user) => {
  ...
  try {
    const { name, email, phone, licenseNumber, password } = await request.json();
...
export const PATCH = withRBAC(async (request, user) => {
  ...
  try {
    ...
    await prisma.user.update({ where: { id }, data: { isActive: status === "active" } });
    ...
    await createNotificationBestEffort(prisma, {
      ...
      message: {
        ar: status === "active" ? "تم تفعيل حساب الطبيب الخاص بك." : "تم تعطيل/رفض حساب الطبيب الخاص بك.",
        en: status === "active" ? "Your doctor account has been activated." : "Your doctor account has been disabled/rejected.",
