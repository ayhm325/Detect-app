...
export async function GET(request) {
  try {
    ...
    const includePending = url.searchParams.get("includePending") === "true" || process.env.DEBUG_INCLUDE_PENDING === "true";

    const whereClause = includePending ? { status: { in: ['active', 'pending'] } } : { status: 'active' };
