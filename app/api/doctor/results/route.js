  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        details: { endpoint: "GET /api/doctor/results" },
      });
    ...
    try {
      ...
      const out = records.map((r) => ({
        ...
        patient: r.patient
          ? {
              ...
              clinicalStatus: r.patient.clinicalStatus || null,
            }
          : null,
        patientClinicalStatus: r.patient?.clinicalStatus || null,
        doctor: r.doctor?.user
          ? { id: r.doctor.userId, name: r.doctor.user.fullName || null }
          : null,
      }));
      logAudit({
        event: "doctor_results_viewed",
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
      ...
      logAudit({
        event: "doctor_results_view_error",
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
        details: { endpoint: "PATCH /api/doctor/results" },
      });
      return Response.json({ error: "Rate limit exceeded" }, { status: 429 });
    }
    ...
    try {
      ...
      if (!before) {
        logAudit({
          event: "doctor_results_update_denied",
          userId: user.id,
          ip: request.headers.get("x-forwarded-for"),
          details: { id },
        });
      ...
      try {
        ...
        const didAddOrChangeNotes =
          Boolean(nextNotes) && nextNotes !== prevNotes;

        if (patientUserId && (didBecomeReviewed || didAddOrChangeNotes)) {
          const arMsg =
            didBecomeReviewed && didAddOrChangeNotes
      ...
      logAudit({
        ...
        details: {
          id,
          reviewedByDoctor: Boolean(reviewedByDoctor),
          hasNotes: Boolean(doctorNotes),
        },
      });
          ...
          record: record
            ? {
                ...
                doctor: record.doctor?.user
                  ? {
                      id: record.doctor.userId,
                      name: record.doctor.user.fullName || null,
                    }
                  : null,
              }
            : null,
      ...
      logAudit({
        event: "doctor_results_update_error",
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
        details: { error: err.message },
      });
