...
export const PATCH = withRBAC(async (request, user, paramsArg) => {
  ...
  if (rl.limited) {
    logAudit({ event: "rate_limit_exceeded", userId: user.id, ip: request.headers.get('x-forwarded-for'), details: { endpoint: "PATCH /api/doctor/appointments/[id]" } });
  ...
  try {
    ...
    if (action === "cancel" || action === "cancelled") {
      updated = await prisma.appointment.update({ where: { id }, data: { status: "cancelled" } });
      ...
      logAudit({ event: "doctor_appointment_cancelled", userId: user.id, ip: request.headers.get('x-forwarded-for'), details: { appointmentId: id } });
    } else if (action === "confirm" || action === "confirmed" || action === "complete" || action === "completed") {
      ...
      logAudit({ event: "doctor_appointment_completed", userId: user.id, ip: request.headers.get('x-forwarded-for'), details: { appointmentId: id } });
    } else if (action === "reschedule") {
      const nextDateRaw = body.scheduledAt || body.newScheduledAt;
      if (!nextDateRaw) return NextResponse.json({ error: "Missing scheduledAt" }, { status: 400 });
      const nextDate = new Date(nextDateRaw);
      if (Number.isNaN(nextDate.getTime())) return NextResponse.json({ error: "Invalid scheduledAt" }, { status: 400 });

      updated = await prisma.appointment.update({ where: { id }, data: { scheduledAt: nextDate, status: "scheduled" } });
      const patientUserId = appt?.patient?.userId || null;
      if (patientUserId) {
        await createNotificationBestEffort(prisma, {
          userId: patientUserId,
      ...
      logAudit({ event: "doctor_appointment_rescheduled", userId: user.id, ip: request.headers.get('x-forwarded-for'), details: { appointmentId: id, scheduledAt: nextDate.toISOString() } });
    } else {
    ...
    logAudit({ event: "doctor_appointment_patch_error", userId: user?.id, ip: request.headers.get('x-forwarded-for'), details: { error: err.message } });
