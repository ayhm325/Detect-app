  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        details: { endpoint: "GET /api/patient/profile" },
      });
    ...
    try {
      ...
      try {
        const activity = await prisma.activity.findFirst({
          where: { userId: userId, type: "notificationSettings" },
        });
      ...
      logAudit({
        event: "patient_profile_view_error",
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
        details: { endpoint: "PUT /api/patient/profile" },
      });
      return NextResponse.json(
        { error: "Rate limit exceeded" },
        { status: 429 },
        ...
        try {
          ...
          if (existing) {
            await prisma.activity.update({
              ...
              data: { meta: body.notificationSettings },
            });
          } else {
            await prisma.activity.create({
              data: {
                ...
                meta: body.notificationSettings,
              },
            });
          }
        } catch (e) {}
