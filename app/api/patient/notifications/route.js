  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        details: { endpoint: "HEAD /api/patient/notifications" },
      });
  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
        details: { endpoint: "GET /api/patient/notifications" },
      });
  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
        details: { endpoint: "PUT /api/patient/notifications" },
      });
      return NextResponse.json({ error: "rate_limited" }, { status: 429 });
    }
    try {
  ...
  async (request, user) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        userId: user.id,
        ip: request.headers.get("x-forwarded-for"),
        details: { endpoint: "DELETE /api/patient/notifications" },
      });
      return NextResponse.json({ error: "rate_limited" }, { status: 429 });
    }
    try {
