...
export const PATCH = withRBAC(async (request, user, paramsArg) => {
  ...
  try {
    ...
    if (!id || !action) {
      // Debug log for missing parameters
      console.error("PATCH /api/patient/appointments/[id] missing params", { id, action, body, paramsArg });
    ...
    if (!appt || appt.patientId !== patient.id) {
      return NextResponse.json({
        ...
        debug: { appt, patientId: patient.id, apptPatientId: appt?.patientId }
      }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: "Server error", debug: { message: err.message, stack: err.stack } }, { status: 500 });
  }
}, ["patient"]);
