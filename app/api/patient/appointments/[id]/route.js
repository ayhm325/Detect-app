  ...
  async (request, user, paramsArg) => {
    ...
    if (rl.limited) {
      logAudit({
        ...
        details: { endpoint: "PATCH /api/patient/appointments/[id]" },
      });
    ...
    try {
      ...
      if (!id || !action) {
        logAudit({
          event: "patient_appointment_patch_missing_params",
          userId: user.id,
          ip: request.headers.get("x-forwarded-for"),
          details: { id, action },
        });
        return NextResponse.json(
          isProd
            ...
            : {
                error: "Missing parameters",
                debug: { id, action, body, paramsArg },
              },
          { status: 400 },
        );
      }
      ...
      if (!patient) {
        return NextResponse.json(
          isProd
            ...
            : { error: "Patient not found", debug: { userId: user.id } },
          { status: 404 },
        );
      }
      ...
      if (action === "confirm" || action === "confirmed") {
        ...
        logAudit({
          event: "patient_appointment_confirmed",
          userId: user.id,
          ip: request.headers.get("x-forwarded-for"),
        ...
        if (!patientReason) {
          return NextResponse.json(
            isProd
              ...
              : { error: "سبب الإلغاء مطلوب", debug: { body } },
            { status: 400 },
          );
        }
        updated = await prisma.appointment.update({
          where: { id },
          data: { status: "cancelled", patientReason },
        });
        await createNotificationBestEffort(prisma, {
          userId: user.id,
          type: "warning",
        ...
        logAudit({
          ...
          userId: user.id,
          ip: request.headers.get("x-forwarded-for"),
          details: { appointmentId: id, patientReason },
        });
        ...
        logAudit({
          event: "patient_appointment_deleted",
          userId: user.id,
          ip: request.headers.get("x-forwarded-for"),
          details: { appointmentId: id },
        });
        ...
        return NextResponse.json(
          isProd
            ...
            : { error: "Unknown action", debug: { action, body } },
          { status: 400 },
        );
      }
      ...
      logAudit({
        event: "patient_appointment_patch_error",
        userId: user?.id,
        ip: request.headers.get("x-forwarded-for"),
...
export const DELETE = withRBAC(
  async (request, user, paramsArg) => {
    try {
      ...
      logAudit({
        event: "patient_appointment_delete_error",
        userId: user?.id,
        ip: request.headers.get("x-forwarded-for"),
