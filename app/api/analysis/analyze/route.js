...
export async function POST(request) {
  try {
    ...
    try {
      ...
      console.warn('/api/analysis/analyze: revoked check failed', e && e.message);
    }
    ...
    const MAX_FILE_SIZE = Number(process.env.MAX_UPLOAD_BYTES || 10 * 1024 * 1024); // default 10MB
    if (buffer.length > MAX_FILE_SIZE) {
      return NextResponse.json({ error: 'file_too_large', message: `File exceeds maximum size of ${Math.round(MAX_FILE_SIZE / 1024 / 1024)}MB` }, { status: 413 });
    }
    ...
    try {
      ...
      const allowed = new Set(['.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp', '.tif', '.tiff']);
      ...
      if (!safeExt) {
        ...
        if (allowed.has(ext)) safeExt = ext;
        else if (fileObj.mimetype && String(fileObj.mimetype).startsWith('image/')) {
          ...
          else if (fileObj.mimetype.includes('tiff') || fileObj.mimetype.includes('tif')) safeExt = '.tiff';
      ...
      if (!safeExt || (!detectedMime && !fileObj.mimetype)) {
        return NextResponse.json({ error: 'invalid_file_type', message: 'Uploaded file is not a supported image type' }, { status: 400 });
      }

      // if file-type returned a mime, ensure it's an image
      if (detectedMime && !String(detectedMime).startsWith('image/')) {
        return NextResponse.json({ error: 'invalid_file_type', message: 'Detected file type is not an image' }, { status: 400 });
      }
      ...
      console.warn('/api/analysis/analyze: failed to save uploaded file', e && e.message);
    ...
    const finalImageUrl = typeof imageUrl === 'string' ? imageUrl : `/uploads/${fileObj.originalname}`;

    let saved = null;
    let saveError = null;
    try {
      saved = await saveAnalysisResult({ userId, imageUrl: finalImageUrl, analysisData: analysisResult });
    } catch (e) {
    ...
    const responsePayload = { success: true, data: saved ?? analysisResult, saved: Boolean(saved) };
    if (!saved && saveError && process.env.NODE_ENV !== 'production') responsePayload.saveError = saveError;
