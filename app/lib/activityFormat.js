export function formatActivityDescription(activity, locale) {
  ...
  switch (type) {
    ...
    case 'register':
      return isEnglish ? `New user registered: ${details}` : `تسجيل مستخدم جديد: ${details}`;
    case 'add_doctor':
      return isEnglish ? `Added doctor: ${details}` : `إضافة طبيب جديد: ${details}`;
    case 'approve_doctor':
      return isEnglish ? `Approved doctor: ${details}` : `تمت الموافقة على طبيب: ${details}`;
    case 'reject_or_delete_doctor':
      return isEnglish ? `Rejected/removed doctor: ${details}` : `تم رفض أو حذف طبيب: ${details}`;
    case 'update_doctor':
      return isEnglish ? `Updated doctor: ${details}` : `تم تحديث طبيب: ${details}`;
    case 'block_doctor':
      return isEnglish ? `Blocked doctor: ${details}` : `تم حظر طبيب: ${details}`;
    default: {
