...
function validate(obj) {
  ...
  // type checks
  if ('analysis_id' in obj && !_isString(obj.analysis_id)) errors.push('analysis_id:string');
  if ('prediction' in obj && !_isString(obj.prediction)) errors.push('prediction:string');

  if ('confidence' in obj) {
    if (!_isNumber(obj.confidence)) errors.push('confidence:number');
    else if (!(obj.confidence >= 0 && obj.confidence <= 1)) errors.push('confidence:0-1');
  }

  if ('explanation' in obj && !_isString(obj.explanation)) errors.push('explanation:string');
  ...
  if ('model_version' in obj && !_isString(obj.model_version)) errors.push('model_version:string');

  if ('inference_time_ms' in obj && !_isNumber(obj.inference_time_ms)) errors.push('inference_time_ms:number');

  if ('created_at' in obj && !_isDateLike(obj.created_at)) errors.push('created_at:datetime');
