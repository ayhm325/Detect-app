...
export async function proxy(request) {
  ...
  const localeApiMatch = pathname.match(
    /^\/[a-z]{2}\/api\/(admin|doctor|patient)(\/.*|$)/,
  );
  if (localeApiMatch) {
  ...
  try {
    ...
    if (e && e.code === "FS_UNAVAILABLE") {
      ...
      try {
        ...
        console.warn(
          "[proxy] fallback is-revoked check failed",
          e2 && e2.message,
        );
      }
    } else {
  ...
  try {
    ...
    try {
      if (DEBUG_PROXY)
        console.log("[proxy] jwt.verify success, role=", user?.role);
    } catch (e) {}
  } catch (err) {
    try {
      if (DEBUG_PROXY)
        console.log("[proxy] jwt.verify error:", err && err.message);
    } catch (e) {}
    if (isApi) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    } else {
